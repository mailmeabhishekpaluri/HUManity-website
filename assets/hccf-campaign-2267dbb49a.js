...
function updateProgress(totals) {
  ...
  if (config?.page === 'landing') {
    ...
    if (stats) {
      campaignGoalReached = stats.committed >= config.home.goal;
      const again = $('#donate-again');
      again.disabled = campaignGoalReached || activeCheckout;
      again.textContent = campaignGoalReached ? 'Yearly goal reached — thank you' : 'Make another donation →';
    }
...
function setupLanding() {
  ...
  let type = 'once', busy = false, memoryRetry = pendingPayload(), lastConfirmedGift = null;
  const againButton = $('#donate-again');
  ...
  function setBusy(value) {
    ...
    againButton.disabled = value || campaignGoalReached;
  ...
  $('.tax-benefit').addEventListener('click', () => { $('#tax-details').open = true; });
  againButton.addEventListener('click', () => {
    if (busy || memoryRetry || campaignGoalReached || !lastConfirmedGift) return;
    const previous = $('#previous-donation');
    previous.textContent = 'Previous gift confirmed: ' + money(lastConfirmedGift.amount) + ' · ' + lastConfirmedGift.paymentId + '.';
    if (lastConfirmedGift.donationType === 'monthly') previous.append(document.createTextNode(' Your existing monthly subscription continues. This is a separate, new donation.'));
    previous.hidden = false;
    form.reset(); idInput.setCustomValidity(''); type = 'once';
    $$('[data-frequency]').forEach(button => button.setAttribute('aria-pressed', String(button.dataset.frequency === 'once')));
    $('#confirmation').hidden = true; againButton.hidden = true;
    form.hidden = false; status.textContent = ''; retryButton.hidden = true;
    setStep(false); updateAmount();
    amountInput.focus({preventScroll: true});
    $('#donate').scrollIntoView({block: 'start', behavior: 'instant'});
  });
  ...
  async function verify(payload) {
    ...
    try {
      ...
      lastConfirmedGift = result; $('#previous-donation').hidden = true;
      ...
      const taxNote = document.createElement('p'); taxNote.className = 'privacy-small';
      taxNote.textContent = 'HUManity will use your details to issue your donation receipt and 80G documentation.';
      confirmation.appendChild(taxNote);
